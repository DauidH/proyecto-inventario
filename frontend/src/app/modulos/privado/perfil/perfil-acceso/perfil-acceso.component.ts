...
@Component({
  ...
  styleUrls: ['./perfil-acceso.component.css'],
})
export class PerfilAccesoComponent implements OnInit, OnDestroy {
  @Input()
  codUsuario: any;
  ...
  public obtenerAcceso(): void {
    this.suscripcionAcceso = this.accesoService
      .obtenerAccesoPerfil(this.codUsuario)
      .pipe(
        map((respuesta: any) => {
  ...
  public enviarFormulario(formulario: NgForm): void {
    ...
    const nuevoAcceso = new Acceso(this.acceso.codUsuario, this.acceso.correoAcceso, '');
    if (this.cargaClave) {
