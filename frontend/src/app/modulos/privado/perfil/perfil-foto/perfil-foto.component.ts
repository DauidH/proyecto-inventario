...
export class PerfilFotoComponent implements OnInit, OnDestroy {
  @Input()
  codUsuario: any;
    ...
    this.suscripcionImagen = this.imagenService
      ...
      .pipe(
        map((respuesta: Imagen[]) => {
          ...
          this.accesoService.fotoMiniatura = this.fotoFavorita;
          return respuesta;
        }),
        catchError((err) => {
          throw err;
  ...
  public convertirFavorita(favorita: Imagen): void {
    this.suscripcionImagen = this.imagenService
      ...
      .pipe(
        map((respuesta) => {
          mostrarMensaje('success', 'Imagen favorita actualizada ', 'Exito', this.toastr);
          this.obtenerImagenes();
          return respuesta;
        }),
        catchError((err) => {
          mostrarMensaje('error', 'Fallo Cambio de imagen favorita', 'Fallo', this.toastr);
          throw err;
        })
      )
      .subscribe(observadorAny);
  ...
  public borrarImagen(imagen: Imagen): void {
    this.suscripcionImagen = this.imagenService
      ...
      .pipe(
        ...
        catchError((err) => {
          mostrarMensaje('error', 'Error eliminar imagen', 'Error', this.toastr);
          throw err;
        }),
        finalize(() => {
          this.obtenerImagenes();
