...
import { AppMenuService } from './../../../../servicios/plantilla/app-menu.service';
...
export class TopMenuComponent implements OnInit {
  ...
  handleTopMenuMenuFocus() {
    ...
    let speed = 0;
    let fullWidth = 0;
    const controlPrevObj = targetMenu.querySelector(
      '.menu-control-start'
    ) as HTMLElement | null;
    const controlPrevWidth = controlPrevObj ? controlPrevObj.clientWidth : 0;
    const controlNextObj = targetMenu.querySelector(
      '.menu-control-end'
    ) as HTMLElement | null;
    ...
    let elm = targetMenu.querySelector(
  ...
  handleTopMenuDrag(containerClassName: string) {
    ...
    const menuItem = menu
      ? (Array.from(
          menu.querySelectorAll('.menu-item:not(.menu-control)')
        ) as HTMLElement[])
      : [];
    ...
    container.addEventListener('mousemove', (e) => {
      ...
      if (totalMarginLeft <= maxScroll) {
        ...
        const menuControlEnd = menu.querySelector(
          '.menu-control.menu-control-end'
        ) as HTMLElement | null;
        if (menuControlEnd) {
      ...
      if (menuWidth < window.innerWidth) {
        const menuControlStart = menu.querySelector(
          '.menu-control.menu-control-start'
        ) as HTMLElement | null;
        if (menuControlStart) {
  ...
  handleTopMenuControlAction(event: MouseEvent, direction: string) {
    ...
    const containerWidth =
      (document.querySelector('.app-top-menu') as HTMLElement).clientWidth -
      (document.querySelector('.app-top-menu') as HTMLElement).clientHeight * 2;
    let totalWidth = 0;
    let finalScrollWidth = 0;
    const controlPrevObj = obj.querySelector(
      '.menu-control-start'
    ) as HTMLElement | null;
    const controlPrevWidth = controlPrevObj?.clientWidth || 0;
    const controlNextObj = obj.querySelector(
      '.menu-control-end'
    ) as HTMLElement | null;
    ...
    const elms = Array.from(
      obj.querySelectorAll('.menu-item')
    ) as HTMLElement[];
    if (elms) {
      elms.forEach((elm) => {
        if (!elm.classList.contains('.menu-control')) {
    ...
    switch (direction) {
      case 'next':
        ...
        if (widthLeftNext <= containerWidth) {
          ...
          setTimeout(() => {
            (
              obj.querySelector('.menu-control.menu-control-end') as HTMLElement
      ...
      case 'prev':
        ...
        if (widthLeftPrev <= containerWidth) {
          (
            obj.querySelector('.menu-control.menu-control-start') as HTMLElement
          )?.classList.remove('show');
          finalScrollWidth = 0;
        } else {
        ...
        if (!forMobile || (forMobile && document.body.clientWidth < 768)) {
          ...
          menus.forEach((m: HTMLElement) => {
            ...
            if (otherTarget && otherTarget !== target) {
              ...
              const otherTargetMenuItem = otherTarget.closest(
                '.menu-item'
              ) as HTMLElement | null;
