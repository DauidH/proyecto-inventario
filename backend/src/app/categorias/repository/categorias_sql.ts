export const SQL_CATEGORIAS = {
  ...
  OBTENER_CATEGORIAS_PAGINADOR:
    "SELECT c.cod_categoria, c.nombre_categoria, c.descripcion_categoria, c.estado_categoria, \
  ...
  CANTIDAD_CATEGORIAS_BUSCAR:
    "SELECT COUNT(c.cod_categoria) AS cantidad \
    FROM categorias c \
    WHERE UNACCENT(c.nombre_categoria) Ilike UNACCENT('%$1^%')",

  OBTENER_CATEGORIAS_BUSCAR:
    "SELECT c.cod_categoria, c.nombre_categoria, c.descripcion_categoria, c.estado_categoria, \
     (SELECT COUNT(cod_rel_categoria_producto) FROM rel_categorias_productos WHERE cod_categoria = c.cod_categoria) AS cantidad_producto \
     FROM categorias c \
     WHERE UNACCENT(c.nombre_categoria) Ilike UNACCENT('%$1^%') \
  ...
  OBTENER_CATEGORIA_VALIDO:
    "SELECT count(cod_categoria) AS valido \
     FROM categorias c \
  ...
  OBTENER_CATEGORIAS_PAGINADOR_BUSCAR:
    "SELECT c.cod_categoria, c.nombre_categoria, c.descripcion_categoria, c.estado_categoria, \
     (SELECT COUNT(cod_rel_categoria_producto) FROM rel_categorias_productos WHERE cod_categoria = c.cod_categoria) AS cantidad_producto \
     FROM categorias c \
     WHERE UNACCENT(c.nombre_categoria) Ilike UNACCENT('%$1^%') \
